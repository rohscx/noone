...
module.exports = function (networkId,apiKey,deviceSn,dstIp,userDefinedDstIp) {
    ...
    const networkStatusFunction = (data) => {
        ...
        // filter out 0 in latency
        const latencyArray = data.map(({latencyMs}) => latencyMs).filter((f) => f > 0);
    ...
    return asyncRequest(options).then(networkStatusFunction).catch(console.log);
}