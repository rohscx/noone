...
module.exports = function (networkId,apiKey,deviceSn,dstIp,userDefinedDstIp) {
    ...
    const networkStatusFunction = (data) => {
        ...
        const lossStdSigma = 1;
        const latencyStdSigma = 2;
        const lossHealthStatus = lossAverage < (lossStd * lossStdSigma) ? "nominal" : "notNominal";
        const latencyHeathStatus = latencyAverage < (latencyStd * latencyStdSigma) ? "nominal" : "notNominal";
