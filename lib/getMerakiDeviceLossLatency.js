...
module.exports = function (networkId,apiKey,deviceSn,dstIp,userDefinedDstIp) {
    ...
    const networkStatusFunction = (data) => {
        ...
        const latencyMsUpperThreshold = 300;
        const latencyMsLowerThreshold = 30;
        ...
        const latencyHeathStatus = (latencyStd * latencyStdSigmaThreshold) <= 0.9 ? "nominal"
            :latencyAverage < latencyMsLowerThreshold ? "nominal" 
            :latencyAverage > latencyMsUpperThreshold ? "notNominal" 
            :latencyAverage < (latencyStd * latencyStdSigmaThreshold) ? "nominal" : "notNominal";
        const metaData = {
            lossAverage,
            lossAverageUnit:"percent",
            latencyAverage,
            latencyAverageUnit:"ms",
            lossStd,
            lossStdSigmaThreshold,
            lossPercentThreshold,
            latencyStd,
            latencyStdSigmaThreshold,
            latencyMsUpperThreshold,
            latencyMsLowerThreshold,
            optionsDstIp
        };
        const response = {
            lossHealthStatus,
