  ...
  if (keyWord){
    ...
    .toLowerCase()
      .search(keyWord[0]
          .trim()
          .toLowerCase()) != -1);
    const dataBlob = JSON.stringify([...filteredData,...assetSearchArray],null,'\t')
    console.log("***SEARCHED**",dataBlob)
    let stringified;
    if (dataBlob.length > 0) {
      
      stringified = 'hit1'//JSON.stringify(dataBlob,null,'\t');
      return dataBlob;
    } else {
      stringified = 'hit2'//JSON.stringify(dataBlob,null,'\t');
      return dataBlob;
    }
  } else {
    stringified = 'hit3'//JSON.stringify(data,null,'\t');
    return data;
  }
}