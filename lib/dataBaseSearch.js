...
module.exports = async function (data,objectKey= "description",messageText) {
  ...
  if (keyWord){
    ...
    const filterEmptyObjects = (data) => {
      ...
      return data.filter((f) => Object.keys(f).length > 0).filter((f) => !f.every(isUndefined));
    }
    if (tagsFiltered.length > 0)assetSearchArrayFiltered.push(filterEmptyObjects(tagsFiltered));
    if (namesFiltered.length > 0)assetSearchArrayFiltered.push(filterEmptyObjects(namesFiltered));
