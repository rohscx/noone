...
module.exports = async function (data,objectKey= "description",messageText) {
  ...
  if (keyWord){
    ...
    const filterEmptyObjects = (data) => {
      const isUndefined = (currentValue) =>  Object.keys(currentValue).map((d) => data[d] === undefined); 
      const simpleFilter = data.filter((f) => Object.keys(f).length > 0);
      const undefinedFilter = simpleFilter.every(isUndefined);
      console.log(data,undefinedFilter)
      return undefinedFilter;
    }
    if (!filterEmptyObjects(tagsFiltered))assetSearchArrayFiltered.push(tagsFiltered);
    if (!filterEmptyObjects(tagsFiltered))assetSearchArrayFiltered.push(namesFiltered);
