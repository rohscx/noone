...
module.exports = async function (data,objectKey= "description",messageText) {
  ...
  if (keyWord){
    ...
    : [];
    console.log(tags,names,tagsFiltered,namesFiltered)
    ...
    const filterEmptyObjects = (data) => {
      ...
      // may need to be changed. What if it could filter on the false stuff...
      return allUndefined.pop();
    }
    ...
    .toLowerCase()
      .search(keyWord[0]
          .trim()
          .toLowerCase()) != -1);
    const dataBlob = [...filteredData,{metaData:flattenArray(assetSearchArrayFiltered)}]; /// ...just be cause it's nested
