...
module.exports = async function (data,objectKey= "description",messageText) {
  ...
  if (keyWord){
    ...
    : [];
    const assetSearchArrayFiltered = [...tagsFiltered,...namesFiltered];
    const filteredData = data.filter((data) => String(data[objectKey])
    .toLowerCase()
      .search(keyWord[0]
          .trim()
          .toLowerCase()) != -1);
    const dataBlob = [...filteredData,{metaData:[...assetSearchArrayFiltered].filter((f) => Object.keys(f).length > 0}];
