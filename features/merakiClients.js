...
module.exports = function(controller) {
  ...
  controller.hears(
    ...
    async function (bot, message) { 
      ...
      if (message.type === "direct_mention") {
        await bot.startConversationInThread(message.channel, message.user, message.incoming_message.channelData.ts);
        await bot.startTyping(message, `${count} clients are online`);
      } else {
        await bot.startTyping(message, `${count} clients are online`);
      }
  ...
  controller.hears(
    ...
    async function (bot, message) { 
      ...
      const count = data.length;
      await bot.startTyping(message, `${count} wired clients are online`);
    });
  ...
  controller.hears(
    ...
    async function (bot, message) { 
      ...
      if (message.type === "direct_mention") {
        await bot.startConversationInThread(message.channel, message.user, message.incoming_message.channelData.ts);
        await bot.startTyping(message, `${count} wireless clients are online`);
      } else {
        await bot.startTyping(message, `${count} wireless clients are online`);
      }
  ...
  controller.hears(
    ...
    async function (bot, message) { 
      if (message.type === "direct_mention") {
        await bot.startConversationInThread(message.channel, message.user, message.incoming_message.channelData.ts);
        await bot.startTyping(message, 'Sure, just Direct Message me an iPv4 Address'); 
      } else {
        await bot.startTyping(message, 'Sure, just Direct Message me an iPv4 Address'); 
      }
    });
